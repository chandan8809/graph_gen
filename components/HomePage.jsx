...
import { LineChart, PieChart, BarChart2, TrendingUp, Activity, AreaChart, Maximize, ChevronRight, GitBranch, Network, Database, Box, Layers, Share2, GitFork, GitCommit, Boxes, Cube, Grid3X3 } from 'lucide-react';
...
const ChartVisualizer = () => {
  ...
  const threeDChartTypes = [
    { 
      ...
      icon:  <Box className="h-8 w-8" style={{color: '#FF9A9E'}}/>,
      label: '3D Bar Chart',
      gradient: 'from-[#6C63FF] to-[#8F7FFF]'
    },
    { 
      type: 'contour',
      icon: <Grid3X3 className="h-8 w-8" style={{color: '#FF6584'}}/>,
      label: '3D Contour Plot',
      gradient: 'from-[#FF6584] to-[#FF85A2]'
    },
    { 
      type: 'mesh',
      icon: <Network className="h-8 w-8" style={{color: '#43A047'}}/>,
      label: '3D Mesh Plot',
      gradient: 'from-[#43A047] to-[#66BB6A]'
    },
    { 
      type: 'network',
      icon: <Share2 className="h-8 w-8" style={{color: '#FF8A65'}}/>,
      label: '3D Network Graph',
      gradient: 'from-[#FF8A65] to-[#FFAB91]'
    },
    { 
      type: 'scatter',
      icon: <GitCommit className="h-8 w-8" style={{color: '#42A5F5'}}/>,
      label: '3D Scatter Plot',
      gradient: 'from-[#42A5F5] to-[#64B5F6]'
    },
    { 
      type: 'surface',
      icon: <Layers className="h-8 w-8" style={{color: '#EC407A'}}/>,
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#EEF2FF] via-white to-[#E0E7FF] text-gray-800 overflow-hidden">
      ...
      <header className="container mx-auto px-6 py-4 flex justify-between items-center relative z-10 backdrop-blur-sm bg-white/30">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          ...
          <button onClick={() => scrollToSection('3d-chart-selection')} className="text-blue-600 hover:text-indigo-600 transition-all hover:scale-105 font-medium">3D Charts</button>
        </nav>
      </header>
        ...
        <div className="flex flex-col md:flex-row items-center">
          <div className="md:w-1/2 md:pr-12">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <button 
                onClick={() => scrollToSection('3d-chart-selection')}
                className="inline-flex items-center py-4 px-8 rounded-full bg-gradient-to-r from-purple-500 via-violet-500 to-fuchsia-600 text-white text-lg font-semibold shadow-xl hover:shadow-2xl transition transform hover:-translate-y-1 hover:scale-105 group"
              >
                Create 3D Chart
                <ChevronRight className="ml-2 w-6 h-6 group-hover:ml-3 transition-all" />
              </button>
      ...
      {/* Select 3D Chart Section */}
      <section id="3d-chart-selection" className="container mx-auto px-6 py-16 relative z-10">
        <h2 className="text-4xl md:text-5xl font-bold text-center mb-16 text-gray-900">
          Select Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-500 via-violet-500 to-fuchsia-600">3D Chart Type</span>
        </h2>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {threeDChartTypes.map((chart, index) => (
            <div 
              onClick={() => router.push({pathname: `/3dchart/${chart.type}`})}
              key={index}
              className="bg-white/80 backdrop-blur-sm rounded-xl p-6 shadow-xl border border-purple-100 hover:shadow-2xl transition transform hover:-translate-y-2 cursor-pointer group"
            >
              <div className={`bg-gradient-to-r ${chart.gradient} rounded-lg inline-block p-3 mb-4 shadow-lg group-hover:scale-110 transition-transform`}>
                {chart.icon}
              </div>
              <h3 className="text-xl font-bold mb-2 text-gray-800">{chart.label}</h3>
              <p className="text-gray-600">Perfect for {chart.type === 'bar' ? 'visualizing data with height, width and depth' : 
                chart.type === 'contour' ? 'displaying three-dimensional surfaces on a two-dimensional plane' : 
                chart.type === 'mesh' ? 'creating three-dimensional surface plots with wireframes' : 
                chart.type === 'network' ? 'visualizing complex network relationships in 3D space' :
                chart.type === 'scatter' ? 'displaying relationships between three variables in 3D space' :
                'visualizing complex mathematical surfaces and topographies'}</p>
            </div>
          ))}
        </div>
      </section>
